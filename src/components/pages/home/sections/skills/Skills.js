...
export default function Skills(props) {
  ...
  const additionalSkills = ADDITIONAL_SKILLS.map((skill) => {
    return (
      <li className='p-1 w-25 m-1 list-inline-item text-white' key={skill.id}>
        {skill.name}
      </li>
    );
  });
  ...
  return (
    <Container id='skills'>
      <Row className='m-5 p-0 justify-content-center align-items-center'>{skillIcons}</Row>
      <Row className='m-5 p-0 justify-content-center align-items-center'>
        <Col xs='12' className='my-0 p-0'>
          <ul className='list-unstyled list-inline text-center'>{additionalSkills}</ul>
        </Col>
      </Row>
    </Container>
  );
