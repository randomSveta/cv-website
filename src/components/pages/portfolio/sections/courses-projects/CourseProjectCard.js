  ...
  return (
    <Card className='rounded-0'>
      <CardHeader className='text-center rounded-0 text-white'>
        <CardLink
          className='app-link app-link-underscore'
          href={props.project.courseLink}
          target='_blank'
          rel='noopener noreferrer'
        >
          <b>{props.project.courseName}</b>
        </CardLink>
        <br />
      ...
      {props.project.image.default ? (
        <a href={props.project.projectLink} target='_blank' rel='noopener noreferrer'>
          <CardImg className='card-image rounded-0' top src={props.project.image.default} alt='Project image' />
        </a>
      ) : (
        ''
      )}
