    ...
    const navigationItems = PAGES.filter(page => !page.article).map(link => {
        return (
            <NavItem key={link.id} className="text-center p-0 ml-2 mr-1 my-1">
                <NavLink exact activeClassName='active-item' className="link-style nav-link-style underscore m-1 p-0 text-decoration-none d-inline-block" to={process.env.PUBLIC_URL + link.url}>
                    {link.name}
                </NavLink>
            </NavItem>
        );
